...
const VenueLocationsTable: FC<VenueLocationsTableProps> = ({
  venueLocations,
  areas,
  venue,
  onAction,
  onSelectionChanged,
  onVenueLocationPatch
}) => {
  ...
  const handleSelectOne = (
    ...
    if (!selectedVenueLocations.includes(id)) {
      setSelectedVenueLocations((prevSelected) => [...prevSelected, id]);
    } else {
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              {venue &&
                venue.inVenueLocationHierarchy1 &&
                venue.inVenueLocationHierarchy1.length > 0 && (
                  <TableCell>{venue.inVenueLocationHierarchy1}</TableCell>
                )}
              {venue &&
                venue.inVenueLocationHierarchy2 &&
                venue.inVenueLocationHierarchy2.length > 0 && (
                  <TableCell>{venue.inVenueLocationHierarchy2}</TableCell>
                )}
              {venue &&
                venue.inVenueLocationHierarchy3 &&
                venue.inVenueLocationHierarchy3.length > 0 && (
          ...
          <TableBody>
            {venueLocations.map((venueLocation, index) => {
              const isSelected = selectedVenueLocations.includes(
                venueLocation.id
              );
              const area = areas.find(
                (x) => x.id === venueLocation.distributionAreaId
              );
              return (
                <TableRow hover key={venueLocation.id}>
                  ...
                  <TableCell>
                    <Switch
                      checked={venueLocation.active || false}
                      onChange={(e) => {
                        handleVenueLocationPatch(
                          venueLocation,
                          'active',
                  ...
                  <TableCell>
                    <Switch
                      checked={venueLocation.deliveryEnabled || false}
                      onChange={(e) => {
                        handleVenueLocationPatch(
                          venueLocation,
                          'deliveryEnabled',
                  ...
                  <TableCell>
                    <Switch
                      checked={venueLocation.pickupEnabled || false}
                      onChange={(e) => {
                        handleVenueLocationPatch(
                          venueLocation,
                          'pickupEnabled',
                  ...
                  <TableCell align="right" padding="checkbox">
                    ...
                    >
                      <MenuItem
                        onClick={() => handleCloseAction('Edit', venueLocation)}
                      >
                        Edit
                      </MenuItem>
                      <MenuItem
                        onClick={() =>
                          handleCloseAction('Delete', venueLocation)
                        }
                      >
                        Delete
                      </MenuItem>
