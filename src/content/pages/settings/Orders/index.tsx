...
import { pauseFor, refreshPause } from 'src/reducers/setting/action';
...
interface OrdersSettingProp {
  ...
  pauseFor: Function;
  refreshPause: Function;
  token: string;
  setVendorStand: Function;
}

const OrderSettings: FC<OrdersSettingProp> = ({
  token,
  pauseUntil,
  vendorStand,
  pauseFor,
    ...
    <>
      ...
      <Container maxWidth="sm">
        <Grid item xs={12} sx={{ mt: 4 }}>
          <Typography variant="subtitle1">Pause orders</Typography>
          {seconds > 0 && (
            <ColoredBox
              sx={{ pt: 2, pl: 2, pb: 1, mt: 2 }}
              className="border-bottom"
            >
              <Grid container spacing={1} alignItems="center">
                <Grid item>
                  <PauseCircleFilledOutlinedIcon color="warning" />
                </Grid>
                <Grid item className="py-0 my-auto" sx={{ flex: 1 }}>
                  <Typography component="span" variant="subtitle1">
                    Paused orders for {Math.floor(seconds / 60)}:
                    {(seconds % 60).toString().padStart(2, '0')} min
                  </Typography>
                </Grid>
                <Grid item sx={{ mr: 2 }}>
                  <Button
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sx={{ mt: 4 }}>
            ...
            <BorderedBox>
              <Switch
                checked={(vendor && vendor.available) || false}
                onChange={(e) => {
            ...
            <BorderedBox>
              <Switch
                checked={(vendor && vendor.pickupAvailable) || false}
                onChange={(e) => {
            ...
            <BorderedBox>
              <Switch
                checked={(vendor && vendor.deliveryAvailable) || false}
                onChange={(e) => {
          ...
          <Grid item xs={12} sx={{ mt: 4 }}>
            <Typography variant="subtitle1">
              Set the total number of orders you can fulfil every 10 minutes.
            ...
            <BorderedBox>
              {vendor && (
                <InputSlider
                  defaultValue={(vendor && vendor.orderCapacity) || 0}
                  maxValue={30}
                  onChange={(v) => {
          ...
          <Grid item xs={12} sx={{ mt: 0 }}>
            <Typography variant="subtitle1">
              Set the total number of customers you want to notify to pick up
              their orders at any one time.
            </Typography>
            <BorderedBox>
              {vendor && (
                <InputSlider
                  maxValue={30}
                  defaultValue={(vendor && vendor.pickupQueueCapacity) || 0}
                  onChange={(v) => {
...
export default connect(reduxState, { pauseFor, refreshPause, setVendorStand })(
  OrderSettings
);
